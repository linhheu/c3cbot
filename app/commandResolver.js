            ...
            attachments: (
                global.getType(data.data.attachments) === "Array" ?
                    (await Promise.all(data.data.attachments.map(item => {
                        if ((item instanceof stream.Readable) || (item instanceof stream.Duplex) || (item instanceof stream.Transform)) {
                        ...
                        if (global.getType(item) == "Object") {
                            if ((item.attachment instanceof stream.Readable) || (item.attachment instanceof stream.Duplex) || (item.attachment instanceof stream.Transform)) {
                                // Converting stream to buffer
                                return new Promise((resolve, reject) => {
                                    let d = [];
                ...
                if (r.supportedPlatform.includes("*") || r.supportedPlatform.includes(cmdData.rawClient.type)) {
                    try {
                        let executedCMD = await r.exec({
                            ...
                            content: cmdData.data.content,
                            prefix: thisThreadPrefix
                        });
